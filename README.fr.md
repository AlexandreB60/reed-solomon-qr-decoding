...
Le module `fft_radix2.py` implémente un Cooley-Tukey classique, validé sur $F_{17}$. Il n'est pas utilisé par le décodeur, et ne peut pas l'être : le radix-2 exige une racine primitive $2^k$-ième de l'unité, or GF(256)\* est d'ordre 255, impair, donc aucun de ses éléments n'a un ordre pair. 
...
`test.py` vérifie l'aller-retour des transformées, la coïncidence entre la FFT radix-2 et la DFT sur $F_{17}$ et le décodage de 0 à 8 erreurs à positions aléatoires - avec `t` égal au nombre d'erreurs comme avec `t` majorant.
